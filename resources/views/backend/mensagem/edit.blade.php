...
@section('content')
    <div class="container-fluid p-0">
        ...
        {!! Form::model($mensagem, [
            'method' => 'PUT',
            'route' => ['backend.mensagens.update', $mensagem->id],
            'id' => 'faq-form',
        ]) !!}
                ...
                <div class="card flex-fill">
                    ...
                    <div class="card-body {{ $errors->has('name') ? ' has-error' : '' }} has-feedback">
                        <input type="text" id="name" class="form-control" placeholder="Nome"
                            value="{{ $mensagem->name }}" name="name" autofocus required>
                    ...
                    <div class="card-body {{ $errors->has('email') ? ' has-error' : '' }} has-feedback">
                        <input type="email" id="email" class="form-control" placeholder="Email"
                            value="{{ $mensagem->email }}" name="email" required>
                    ...
                    <div class="card-body {{ $errors->has('assunto') ? ' has-error' : '' }} has-feedback">
                        <input type="text" id="assunto" class="form-control" placeholder="Assunto"
                            value="{{ $mensagem->assunto }}" name="assunto" required>
                    ...
                    <div class="card-body {{ $errors->has('mensagem') ? 'has-error' : '' }} has-feedback">
                        <textarea class="form-control @error('mensagem') is-invalid @enderror" rows="2" placeholder="Mensagem"
                            name="mensagem">{{ $mensagem->mensagem }}</textarea>
        ...
        <a class="btn btn-dark text-yellow1" href="{{ route('backend.mensagens.index') }}"><i class="align-middle"
